...
- **Coin stacks**: history sessions untouched for 30 days leave the map and pile up, as a neat
  stack of cell-sized hex tiles, on one hex at the front of their project's cluster. The stack
  grows with the count (one more tile each time it grows by φ) and shows the number on top; hover
  for a summary, right-click to list the sessions and open any of them. Search still finds them,
  and the replay unpiles them.
